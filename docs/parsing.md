...
Ответ @Yaroslav_Schekin (Telegram)
...
2. Синтаксический анализа (syntax analysis, parsing).
...
Существуют и альтернативные методы parsing, для которых наличие lexer либо в принципе не нужно, либо даже вредно (для успешного решения задач, где применяются эти методы). Среди чаще всего используемых подобных методов — написание parsers "вручную" прямо на используемом ЯП с помощью таких методов структурирования программ-парсеров, как recursive descent или parser combinators.
